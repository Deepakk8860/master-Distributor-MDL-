...
import com.android.masterdistributormdl.databinding.ViewCertificateBinding
...
class ViewCertificate : Fragment() {
  private lateinit var binding: ViewCertificateBinding

    lateinit var model: CertificateModel

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?
    ): View {
        binding =
            DataBindingUtil.inflate(inflater, R.layout.view_certificate, container, false)
        model = ViewModelProvider(this)[CertificateModel::class.java]
        binding.model = model
...
class CertificateModel(application: Application) : AndroidViewModel(application) {
    ...
    val id = user.id
    val pdfImagesFolder = File(application.filesDir, "MF-Images")
    val pdfFile = File(application.filesDir, "Certificate-$id.pdf")
    ...
    init {
        val json = sharedPreference.getString("certificate_images")
        if (json!!.isNotEmpty()) {
        ...
        viewModelScope.launch {
            kotlin.runCatching {
                ...
                if (uri.isNullOrEmpty()) {
                    showToastShort(it.message)
                    sharedPreference.putString("certificate_images", "")
    ...
    private fun convertFiletoPdf() {
        ...
        for (i in 0 until pageCount) {
            ...
            page.close()
            val file = File(pdfImagesFolder.absolutePath, "image$i.png")
            try {
        ...
        sharedPreference.putString("certificate_images", Gson().toJson(pdfImage))
