...
class HomeModelLead(application: Application) : AndroidViewModel(application) {
        ...
        viewModelScope.launch {
            kotlin.runCatching {
                ...
                retrofitErrorDist.postValue(
                    com.android.masterdistributormdl.gskDistributor.network.errorRetrofit(
                        it
                    )
                )
            }
    ...
    fun getClientList(param: JsonObject, result: (ClientListResult) -> Unit) {
        ...
        viewModelScope.launch {
            kotlin.runCatching {
                withContext(Dispatchers.IO) {
                    getClient().getClientList(param).body()!!
                }
            }.onSuccess {
                isLoaderVisible.value = false
                result(it)
